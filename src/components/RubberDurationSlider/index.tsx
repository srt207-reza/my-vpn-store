...
export default function RubberDurationSlider({ allowedValues, value, onChange }: RubberDurationSliderProps) {
    ...
    useEffect(() => {
        ...
        if (width <= 0 || isDragging) return;

        const targetX = initialPercentage * width;

        if (!hasPositionedHandleRef.current || shouldReduceMotion) {
            dragX.set(targetX);
            hasPositionedHandleRef.current = true;
            return;
        }
    ...
    }, [width, value, allowedValues, initialPercentage, isDragging, dragX, shouldReduceMotion]);
    ...
    const handlePointerMove = useCallback(
        ...
        [dragX, dragY, allowedValues, maxIndex, onChange]
    );
    ...
    const handlePointerUp = useCallback(() => {
        ...
        if (containerRef.current) {
            const rect = containerRef.current.getBoundingClientRect();
            const snapIndex = allowedValues.indexOf(selectedValueRef.current);
    ...
    }, [dragY, dragX, handlePointerMove, allowedValues, maxIndex]);
